...
class CohortAnalysisAgent(BaseAgent):
    ...
    def execute(self, upstream_results: list | None = None) -> AgentResult:
        ...
        # Store results
        cursor = conn.cursor()
        cursor.execute("DELETE FROM analysis_cohort_analysis")
        cursor.execute(
            """INSERT INTO analysis_cohort_analysis (analyzed_at, record_count)
               VALUES (%s, %s)""",
        ...
        _logger.info("CohortAnalysisAgent: Analyzed %d records", len(results))
