...
class GraphTraversalAgent(BaseAgent):
    ...
    def execute(self, upstream_results: list | None = None) -> AgentResult:
        ...
        # Store results
        cursor = conn.cursor()
        cursor.execute("DELETE FROM analysis_graph_traversal")
        cursor.execute(
            """INSERT INTO analysis_graph_traversal (analyzed_at, record_count)
               VALUES (%s, %s)""",
        ...
        _logger.info("GraphTraversalAgent: Analyzed %d records", len(results))
