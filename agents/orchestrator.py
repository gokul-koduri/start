...
class OrchestratorAgent(BaseAgent):
    ...
    def execute(self, upstream_results: list | None = None) -> AgentResult:
        ...
        agent_names = pipelines.get(
            pipeline_name, pipelines.get("daily", ["collection", "report"])
        )

        _logger.info(
            "Orchestrator: Starting pipeline '%s' with %d stages",
            pipeline_name,
            len(agent_names),
        )
        ...
        if feedback_priority:
            _logger.info(
                "Orchestrator: Feedback priorities detected: %s", feedback_priority
            )
        ...
        for agent_name in agent_names:
            try:
                ...
                all_results.append(
                    AgentResult(agent_name=agent_name, status="failed", errors=[str(e)])
                )
            ...
            if result.status == "failed":
                if continue_on_failure:
                    _logger.warning(
                        "Orchestrator: '%s' failed — continuing (continue_on_failure=True)",
                        agent_name,
                    )
                    overall_status = "partial"
                else:
                    _logger.error(
                        "Orchestrator: '%s' failed — halting pipeline", agent_name
                    )
        ...
        _logger.info("=" * 60)
        _logger.info(
            "ORCHESTRATOR SUMMARY — pipeline '%s' (%.1fs)", pipeline_name, elapsed
        )
        _logger.info("=" * 60)
        for r in all_results:
            _logger.info(
                "  %-25s status=%-8s errors=%d", r.agent_name, r.status, len(r.errors)
            )
