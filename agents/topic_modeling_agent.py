...
class TopicModelingAgent(BaseAgent):
    ...
    def execute(self, upstream_results: list | None = None) -> AgentResult:
        ...
        # Store results
        cursor = conn.cursor()
        cursor.execute("DELETE FROM analysis_topic_modeling")
        cursor.execute(
            """INSERT INTO analysis_topic_modeling (analyzed_at, record_count)
               VALUES (%s, %s)""",
        ...
        _logger.info("TopicModelingAgent: Analyzed %d records", len(results))
