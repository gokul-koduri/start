...
class TrendDetectorAgent(BaseAgent):
    ...
    def execute(self, upstream_results: list | None = None) -> AgentResult:
        ...
        # Store results
        cursor = conn.cursor()
        cursor.execute("DELETE FROM analysis_trend_detection")
        cursor.execute(
            """INSERT INTO analysis_trend_detection (analyzed_at, record_count)
               VALUES (%s, %s)""",
        ...
        _logger.info("TrendDetectorAgent: Analyzed %d records", len(results))
