...
class JWTHandler:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.expiry_hours = self.config.get(
            "jwt_expiry_hours", auth_config.get("jwt_expiry_hours", 24)
        )

        if not HAS_JWT:
            _logger.warning(
                "PyJWT not installed — JWT tokens will use mock implementation"
            )
    ...
    def validate_token(self, token: str) -> Dict[str, Any]:
        ...
        if HAS_JWT:
            ...
            try:
                ...
                # Check expiry
                if (
                    mock_payload.get("exp", 0)
                    < datetime.datetime.now(datetime.timezone.utc).timestamp()
                ):
                    raise ValueError("Token has expired")
                # Remove timestamp fields for consistency with real JWT
                return {
                    k: v for k, v in mock_payload.items() if k not in ["exp", "iat"]
                }
