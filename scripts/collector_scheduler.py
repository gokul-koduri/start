    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if not _is_transient_error(Exception(last_error)):
                _logger.warning(
                    "%s: permanent error (not retrying): %s", collector_name, last_error
                )
                break
...
def run_group(
    collector_names: list[str], config: dict, scheduler_config: dict
) -> list[dict]:
    ...
    results = []
    _logger.info(
        "Running %d collectors (max %d concurrent)",
        len(collector_names),
        max_concurrent,
    )
...
def print_summary(results: list[dict]):
    ...
    _logger.info("-" * 60)
    _logger.info(
        "Total: %d collectors, %d succeeded, %d failed, %d skipped, %d records",
        total,
        success,
        failed,
        skipped,
...
def daemon_loop(config: dict):
    ...
    _logger.info(
        "Scheduler started: %d collectors across %d groups", len(intervals), len(groups)
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Collector scheduler for Opportunity Intelligence Platform"
    )
    parser.add_argument(
        "--once", action="store_true", help="Run all collectors once and exit"
    )
    parser.add_argument(
        "--group", type=str, help="Run a specific group (fast/standard/daily)"
    )
    ...
    try:
        ...
        if args.group:
            ...
            if not group_def:
                ...
                sys.exit(1)
            _logger.info(
                "Running group: %s (%d collectors)",
                args.group,
                len(group_def.get("collectors", [])),
            )
            results = run_group(
                group_def.get("collectors", []), config, scheduler_config
            )
            print_summary(results)
