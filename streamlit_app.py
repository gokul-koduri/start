...
if page == "📊 Overview":
    ...
    if not stats:
        st.warning(
            "No database connection. Run the pipeline first or check MySQL settings."
        )
    ...
    if startups:
        ...
        recent = df.nlargest(10, "year_shutdown")[
            ["name", "sector", "country", "year_shutdown", "failure_category"]
        ]
...
# ── Startup Explorer ─────────────────────────────────────────
    ...
    with st.expander("🔍 Filters", expanded=True):
        ...
        selected_cats = col3.multiselect(
            "Failure Category", categories, default=categories
        )

        year_range = col4.slider(
            "Shutdown Year",
            int(df["year_shutdown"].min()),
    ...
    if startup_names:
        ...
        with col_a:
            ...
            st.markdown(
                f"**Country:** {row.get('country', 'N/A')} | **Region:** {row.get('region', 'N/A')}"
            )
            st.markdown(
                f"**Founded:** {row.get('year_founded', 'N/A')} → **Shutdown:** {row.get('year_shutdown', 'N/A')}"
            )
            funding = row.get("funding_raised_usd")
            st.markdown(
                f"**Funding:** ${funding:,.0f}" if funding else "**Funding:** Unknown"
            )
...
# ── Risk Scores ──────────────────────────────────────────────
    ...
    if not risk_data:
        st.info(
            "No risk scores available. Run: `python run_agent.py --pipeline analysis`"
        )
        st.stop()
    ...
    st.dataframe(
        top_risk[
            ["name", "sector", "country", "risk_score", "risk_level", "recommendation"]
        ],
        use_container_width=True,
        hide_index=True,
    )
    ...
    sector_risk = (
        df.groupby("sector")["risk_score"].mean().sort_values(ascending=False).head(15)
    )
    fig3 = px.bar(
        x=sector_risk.values,
        y=sector_risk.index,
        orientation="h",
        color=sector_risk.values,
...
# ── Sector Analysis ──────────────────────────────────────────
    ...
    with col2:
        ...
        sector_data = (
            df.groupby(["sector", "failure_category"]).size().reset_index(name="count")
        )
        fig2 = px.treemap(
            sector_data,
            path=["sector", "failure_category"],
            values="count",
            color="count",
    ...
    fig3 = px.line(
        ...
        color="sector",
        markers=True,
        labels={"year_shutdown": "Year", "count": "Failures"},
    )
    ...
    if bls:
        ...
        if not ind_data.empty:
            ...
            for col_name, label in [
                ("age_1_yr_survival", "1-Year"),
                ("age_2_yr_survival", "2-Year"),
                ("age_3_yr_survival", "3-Year"),
                ("age_5_yr_survival", "5-Year"),
            ]:
            ...
            fig4.update_layout(
                title=f"Survival Rates: {selected_ind}",
...
# ── Geographic Insights ──────────────────────────────────────
    ...
    fig3 = px.density_heatmap(
        pivot, x="sector", y="country", z="count", color_continuous_scale="YlOrRd"
    )
    ...
    funding_by_country = (
        df[df["funding_raised_usd"] > 0]
        .groupby("country")["funding_raised_usd"]
        .mean()
        .sort_values(ascending=False)
        .head(15)
    )
    fig4 = px.bar(
        x=funding_by_country.values,
        y=funding_by_country.index,
        orientation="h",
        labels={"x": "Avg Funding (USD)", "y": ""},
        color=funding_by_country.values,
...
# ── Knowledge Graph ──────────────────────────────────────────
    ...
    if not entities:
        st.info(
            "No knowledge graph data. Run: `python run_agent.py --pipeline analysis`"
        )
        st.stop()
    ...
    entity_search = st.text_input(
        "Search entities...", placeholder="e.g. Uber, EV, Sequoia"
    )
    if entity_search:
        ...
        if not results.empty:
            ...
            rels = rel_df[
                (rel_df["source_entity_id"] == ent_id)
                | (rel_df["target_entity_id"] == ent_id)
            ]
            if not rels.empty:
                st.subheader(f"Relationships: {results.iloc[0]['name']}")
                st.dataframe(
                    rels[["source_name", "relationship_type", "target_name", "weight"]],
                    use_container_width=True,
                    hide_index=True,
                )
    ...
    top_rels = rel_df[
        rel_df["source_entity_id"].isin(top_ent_ids)
        & rel_df["target_entity_id"].isin(top_ent_ids)
    ]
...
# ── Revival Opportunities ────────────────────────────────────
    ...
    st.dataframe(
        df[["industry", "died_period", "why_returning", "market_fit"]],
        use_container_width=True,
        hide_index=True,
    )
...
# ── LLM Pricing ──────────────────────────────────────────────
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📊 Pricing Comparison", "🏆 Benchmarks", "📉 Price Trends"]
    )
    ...
    with tab1:
        if pricing:
            ...
            with col1:
                fig = px.scatter(
                    df,
                    x="input_price_per_1m",
                    y="output_price_per_1m",
                    color="provider",
    ...
    with tab3:
        ...
        if price_changes:
            st.dataframe(
                pd.DataFrame(price_changes), use_container_width=True, hide_index=True
            )
        else:
...
# ── Pipeline Health ──────────────────────────────────────────
    ...
    st.dataframe(
        df[display_cols], use_container_width=True, hide_index=True, height=400
    )
    ...
    with col2:
        ...
        if not errors.empty:
            ...
            for _, row in errors.head(10).iterrows():
                st.error(
                    f"**{row['agent_name']}** ({row['started_at']}): {row.get('error_message', 'Unknown')}"
                )
        else:
    ...
    if spans:
        ...
        if not anomalies.empty:
            ...
            st.dataframe(
                anomalies[
                    ["agent_name", "duration_seconds", "anomaly_type", "anomaly_detail"]
                ],
                use_container_width=True,
                hide_index=True,
            )
...
# ── Sentiment Analysis Page ──────────────────────────────────
    ...
    avg_score = query_db(
        "SELECT AVG(sentiment_score) as avg FROM news_articles WHERE sentiment_score IS NOT NULL"
    )
    ...
    if scored_count == 0:
        st.info(
            "No articles scored yet. Run the sentiment agent: `python run_agent.py --pipeline analysis`"
        )
    else:
        st.divider()
        ...
        with pos:
            ...
            if positive:
                st.dataframe(
                    pd.DataFrame(positive), use_container_width=True, hide_index=True
                )
            else:
        ...
        with neg:
            ...
            if negative:
                st.dataframe(
                    pd.DataFrame(negative), use_container_width=True, hide_index=True
                )
            else:
        ...
        if models_used:
            ...
            st.dataframe(
                pd.DataFrame(models_used), use_container_width=True, hide_index=True
            )
